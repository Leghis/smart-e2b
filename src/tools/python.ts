...
setInterval(() => {
  ...
  for (const [id, { sandbox, lastUsed }] of sandboxCache.entries()) {
    if (now - lastUsed > 30 * 60 * 1000) {
      console.error(`Closing unused Python sandbox ${id}`);
      try {
...
/**
 * Exécute du code Python dans un sandbox E2B
 */
export async function executePython(args: { code: string, timeout?: number, sandboxId?: string }) {
  const { code, timeout = 180000, sandboxId = 'default-py' } = args; // 3 minutes par défaut
  ...
  try {
    ...
    return await withPipeErrorHandling(async () => {
      ...
      return await withRedirectedStdout(async () => {
        ...
        if (!sandboxEntry) {
          console.error(`Creating new Python sandbox with ID: ${sandboxId}`);
        ...
        console.error(`Exécution du code Python avec un timeout de ${timeout/1000} secondes...`);
        ...
        try {
          // Exécuter le code
          result = await sandboxEntry.sandbox.runCode(code, { language: 'python' });
